    ...
    <div class="content">
      <div class="data-card">
        ...
        <table class="tbl">
          ...
          <tbody>
            <?php if (! empty($conges) && is_array($conges)): ?>
              <?php foreach ($conges as $c):
                $typeLabel = $types[$c['type_conge_id']] ?? '—';
                $start = $c['date_debut'] ? date('j M Y', strtotime($c['date_debut'])) : '—';
                $end = $c['date_fin'] ? date('j M Y', strtotime($c['date_fin'])) : '—';
                $duration = isset($c['nb_jours']) ? ((int)$c['nb_jours']).' j' : '—';
                $statut = $c['statut'] ?? '—';
                $commentaire = ! empty($c['commentaire_rh']) ? $c['commentaire_rh'] : '—';
                $class = 's-attente';
                if ($statut === 'approuvee') { $class = 's-approuvee'; }
                elseif ($statut === 'refusee') { $class = 's-refusee'; }
                elseif ($statut === 'annulee') { $class = 's-annulee'; }
              ?>
                <tr>
                  <td><span class="type-badge"><?= esc($typeLabel) ?></span></td>
                  <td class="td-muted"><?= esc($start) ?></td>
                  <td class="td-muted"><?= esc($end) ?></td>
                  <td class="td-mono"><?= esc($duration) ?></td>
                  <td><span class="statut <?= esc($class) ?>"><?= esc($statut) ?></span></td>
                  <td class="td-muted" style="font-size:.78rem"><?= esc($commentaire) ?></td>
                  <td>
                    <?php if ($statut === 'en_attente'): ?>
                      <form method="post" action="/conges/cancel/<?= (int)$c['id'] ?>" style="display:inline">
